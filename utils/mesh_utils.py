...
class GaussianExtractor(object):
    ...
    @torch.no_grad()
    def reconstruction(self, viewpoint_stack,gt = False,scene=None):
        """
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            ...
            if gt:
                ...
                if depth.shape[1]==640:
                    scale = 5000
                else:
                    scale = 6553.5


                depth = (depth / scale).to(torch.float).cuda().unsqueeze(0)
    ...
    @torch.no_grad()
    def cal_scale(self, gt_depth,depth):
        gt_depth_np = np.array(gt_depth)
        depth_np = np.array(depth)
        depth_np[gt_depth_np == 0] = 0
        gt_depth_np_flatten = gt_depth_np.flatten().astype(np.float32)
        depth_np_flatten = depth_np.flatten()
        fenzi = 0
        fenmu = 0
        for i in range(depth_np_flatten.__len__()):
            fenzi = fenzi + depth_np_flatten[i]*gt_depth_np_flatten[i]
            fenmu = fenmu + gt_depth_np_flatten[i] ** 2

        scale= fenzi/fenmu
        depth_np_scaled = depth_np / scale

        gt_depth_np_vis = gt_depth_np/gt_depth_np.max() * 255
        depth_np_scaled_vis = depth_np_scaled/gt_depth_np.max() * 255

        # plt.imshow(gt_depth_np_vis)
    ...
    def export_image(self, path):
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            # scale = self.cal_scale(gt_depth,self.depthmaps[idx][0].cpu().numpy())
            # print("当前帧的scale: ",scale)
            # scales.append(scale)
